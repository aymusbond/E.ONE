...
public class Gun : MonoBehaviourPunCallbacks
{
    ...
    void RPCShooting()
    {
        ...
        if(Input.GetMouseButtonDown(0))
        {
            ...
            {
                if(hit.transform.gameObject == PhotonNetwork.IsMasterClient)
                {
                    text.text = "Master Client Hit!";
                }
                else if(hit.transform.gameObject == (!PhotonNetwork.IsMasterClient))
                {
                    text.text = "Client Hit!";
                }
            }
        }
    }
